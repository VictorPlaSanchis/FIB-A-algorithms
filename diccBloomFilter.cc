...
class diccBloomFilter : EstructuraDades {
    ...
    public:
        ...
        diccBloomFilter(){
            this->bitsArray = vector<bool>(0);
            this->n=0;
            this->m=0;
            this->k=0;
            this->p = 0.01;
            this->wordsToAdd = vector<string>(0);
        ...
        int lengthOfArray(){
            return -(this->n * log(this->p))/(pow(log(2),2)) * 30;
        }
        ...
        SearchResult findWord(string word){
            if(this->m == 0) return SearchResult::WORD_NOT_FOUND;
